...
namespace blank
{
    public partial class VecPaintV : UserControl
    {
        ...
        private Pen pen_triangle = new Pen(Color.Blue, 2f);
        ...
        {
            var triangles = PolygonTriangulation.TriangulateMonotonePolygon(PolygonToPointList(cur_edit_polygon));
        ...
        private List<Point2D> PolygonToPointList(Polygon polygon)
        {
            List<Point2D> res = new List<Point2D>();

            Vertex start = polygon.Front;

            res.Add(polygon.Point);
            polygon.Advance(Vertex.ROTATION.CLOCKWISE);
